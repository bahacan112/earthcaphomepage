...
export default function FeaturedSavingsSection() {
  const { featuredSavingsSection } = languageData;
              ...
              <div className="mt-6">
                <div className="text-4xl font-semibold text-slate-900">
                  ...
                  <span className="ml-2 align-middle text-sm font-medium uppercase text-slate-500">
                    {featuredSavingsSection.labels.aer}
                  </span>
                </div>
              </div>

              {/* Term & Country */}
              <div className="mt-3 text-sm text-slate-600">{acc.term}</div>

              {/* Chip: No withholding tax */}
              {acc.withholdingTax && (
                <div className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-gray-100 px-3 py-1 text-xs font-medium text-slate-700">
                  <ShieldCheck size={14} className="text-slate-700" />
                  {featuredSavingsSection.labels.noWithholdingTax}
                </div>
              )}
            </div>
          ))}
